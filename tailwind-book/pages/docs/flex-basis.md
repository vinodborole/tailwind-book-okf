...
| Class | Styles | 
|---|---|
| `basis-` | `flex-basis: calc(var(--spacing) * ` | 
| `basis-` | `flex-basis: calc(` | 
...
`<div class="basis-[30vw] ...">  <!-- ... --></div>`For CSS variables, you can also use the `basis-(` syntax:`<custom-property>`)

`<div class="basis-(--my-basis) ...">  <!-- ... --></div>`This is just a shorthand for `basis-[var(` that adds the `<custom-property>`)]`var()` function for you automatically.
...
`@theme {  --container-4xs: 14rem; }`Now the `basis-4xs` utility can be used in your markup:
