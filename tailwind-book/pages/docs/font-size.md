...
`<p class="text-[14px] ...">  Lorem ipsum dolor sit amet...</p>`For CSS variables, you can also use the `text-(length:` syntax:`<custom-property>`)

`<p class="text-(length:--my-text-size) ...">  Lorem ipsum dolor sit amet...</p>`This is just a shorthand for `text-[length:var(` that adds the `<custom-property>`)]`var()` function for you automatically.
...
`@theme {  --text-tiny: 0.625rem; }`Now the `text-tiny` utility can be used in your markup:
