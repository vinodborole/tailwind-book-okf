...
For CSS variables, you can also use the `max-h-(` syntax:`<custom-property>`)

`<div class="max-h-(--my-max-height) ...">  <!-- ... --></div>`
This is just a shorthand for `max-h-[var(` that adds the `<custom-property>`)]`var()` function for you automatically.
