...
For CSS variables, you can also use the `grid-cols-(` syntax:`<custom-property>`)

`<div class="grid-cols-(--my-grid-cols) ...">  <!-- ... --></div>`
This is just a shorthand for `grid-cols-[var(` that adds the `<custom-property>`)]`var()` function for you automatically.
