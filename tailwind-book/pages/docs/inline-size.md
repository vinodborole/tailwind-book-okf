...
| Class | Styles | 
|---|---|
| `inline-` | `inline-size: calc(var(--spacing) * ` | 
| `inline-` | `inline-size: calc(` | 
...
`<div class="inline-screen">  <!-- ... --></div>`Use the `inline-auto` utility to remove an element's assigned inline size under a specific condition, like at a particular breakpoint:

`<div class="inline-full md:inline-auto">  <!-- ... --></div>`Use the `inline-[` syntax to set the inline size based on a completely custom value:`<value>`]

`<div class="inline-[5px] ...">  <!-- ... --></div>`For CSS variables, you can also use the `inline-(` syntax:`<custom-property>`)

`<div class="inline-(--my-inline-size) ...">  <!-- ... --></div>`This is just a shorthand for `inline-[var(` that adds the `<custom-property>`)]`var()` function for you automatically.
